...
async function fetchGuideViewCount(guideName) {
    try {
        ...
        const response = await fetch(`https://heatlabs-pixel-api.vercel.app/api/stats?image=heatlabs-tracker-pixel-${baseName}.png`);
        if (!response.ok) {
...
function updateCardDates(cards) {
    cards.forEach(card => {
        const dateElement = card.querySelector('.guide-meta span:first-child');
        if (dateElement) {
...
// Function to sort and filter guide cards
function updateguideDisplay() {
    ...
    if (originalCards.length === 0) {
        originalCards = Array.from(guideGrid.querySelectorAll('.guide-card'));
    ...
    // Clear the grid
    while (guideGrid.firstChild) {
        guideGrid.removeChild(guideGrid.firstChild);
    }

    // Add paginated cards back to the grid
    paginatedCards.forEach(card => {
        const clonedCard = card.cloneNode(true);
        guideGrid.appendChild(clonedCard);
    });
