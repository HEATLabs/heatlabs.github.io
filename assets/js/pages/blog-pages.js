document.addEventListener('DOMContentLoaded', function() {
    ...
    fetchBlogViewCount().then(views => {
        const blogMeta = document.querySelector('.blog-meta');
        if (blogMeta) {
            const viewCounter = document.createElement('span');
            viewCounter.className = 'blog-views-counter';
            viewCounter.innerHTML = `
                <i class="fas fa-eye"></i>
                <span class="blog-views-count">${views.totalViews.toLocaleString()}</span> views
            `;
            blogMeta.appendChild(viewCounter);
        }
    });

    // Initialize any interactive elements specific to blog pages
    initializeBlogPageElements();
});

// Function to fetch view count from API
async function fetchBlogViewCount() {
    try {
...
function initializeBlogPageElements() {
    // Initialize any blog-specific interactive elements here
    ...
    if ('IntersectionObserver' in window) {
        ...
        document.querySelectorAll('.blog-image, .sidebar-card').forEach(el => {
            observer.observe(el);
        });
    }
}